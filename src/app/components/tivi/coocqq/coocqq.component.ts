...
@Component({
  selector: 'app-coocqq',
  templateUrl: './coocqq.component.html',
  styleUrls: ['./coocqq.component.css']
})
export class CoocqqComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    const questionId = '14'; // Replace with the desired ID
  ...
  refreshPage(): void {
    if (this.shouldRefreshProducts) {
      ...
      this.tiviService.getTiviIds([7, 8, 9]).subscribe(data => {
        this.allqleds = data;
