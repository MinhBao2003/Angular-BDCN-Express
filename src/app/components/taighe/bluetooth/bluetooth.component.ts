...
export class BluetoothComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.taigheService.getHeadphone(phoneIds).subscribe(data => {
      this.taighes = data;
    });
    const questionId = '16'; // Replace with the desired ID
  ...
  refreshPage(): void {
    if (this.shouldRefreshProducts) {
      ...
      this.taigheService.getHeadphone([1,2,3]).subscribe(data => {
        this.allPhones = data;
  ...
  onRomSelect(): void {
    if (this.selectedRom !== '') {
      ...
      // Reset the filter and fetch all phones
      this.taigheService.getHeadphone([10,11,12]).subscribe(data => {
        this.allPhones = data;
  ...
  filterRomPhones(): void {
    // Filter phones based on selected RAM
    this.taighes = this.allPhones.filter(phone => phone.rom === this.selectedRom);
