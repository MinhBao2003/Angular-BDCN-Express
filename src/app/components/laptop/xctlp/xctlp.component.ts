...
@Component({
  selector: 'app-xctlp',
  templateUrl: './xctlp.component.html',
  styleUrls: ['./xctlp.component.css']
})
export class XctlpComponent implements  OnInit{
  latops: any[] = []; // Chứa danh sách điện thoại từ API
  ...
  constructor(private yourService: QuestionService,private laptopService: LaptopService,private route: ActivatedRoute) {}

  ngOnInit() {
    this.laptopService.getLaptops().subscribe(data => {
      this.latops = data;
    });
    const questionId = '11'; // Replace with the desired ID
  ...
  filterByRam(minRam: number): void {
    ...
    if (minRam === 12) {
      this.latops = this.allPhones.filter(
  ...
  filterByRom(minRom: number): void {
    ...
    if (minRom === 256) {
      this.latops = this.allPhones.filter(
        (phone) => parseFloat(phone.rom) >= minRom || phone.rom === '512GB'
      );
    } else {
      this.latops = this.allPhones.filter(
  ...
  filterPhones(): void {
    // Filter phones based on selected RAM
    this.latops = this.allPhones.filter(phone => phone.ram === this.selectedRam);
  }
  ...
  filterRomPhones(): void {
    // Filter phones based on selected RAM
    this.latops = this.allPhones.filter(phone => phone.rom === this.selectedRom);
  ...
  filterInchesPhones(): void {
    // Filter phones based on selected RAM
    this.latops = this.allPhones.filter(phone => phone.screenSize === this.selectedInches);
  }

  sortByPriceDescending(): void {
    this.latops.sort((a, b) => {
