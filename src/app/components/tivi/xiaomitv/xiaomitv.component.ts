...
@Component({
  selector: 'app-xiaomitv',
  templateUrl: './xiaomitv.component.html',
  styleUrls: ['./xiaomitv.component.css']
})
export class XiaomitvComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    const questionId = '13'; // Replace with the desired ID
  ...
  refreshPage(): void {
    if (this.shouldRefreshProducts) {
      ...
      this.tiviService.getTiviIds([4, 5, 6]).subscribe(data => {
        this.allqleds = data;
